...
Everything except `complete` can be picked up with `/goal resume` (a fresh budget of the same size, same objective; `/goal resume --max N` changes the size). The last goal is saved with the session, so it survives a restart: after `fuseraft repl --resume <id>` or `/switch`, an unfinished goal is announced and `/goal resume` picks it up. A goal that was still running when the process died comes back as `interrupted`. `/goal drop` forgets the last goal, so a later message is an ordinary turn. `--max 8` sets the budget: `/goal --max 8 <objective>`. HITL, the sandbox and safe mode apply to every turn exactly as usual, and Ctrl+C stops the loop. Each audit is recorded as a `goal_audit` event.
