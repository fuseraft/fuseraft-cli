...
| SubAgent | `subagent_explore` / `subagent_locate` / `subagent_delegate` — the same tools behind [`/explore`, `/locate` and `/delegate`](#sub-agents-and-getting-unstuck), callable mid-turn; plus `subagent_run` when you've defined [custom sub-agents](sub-agents.md) |
