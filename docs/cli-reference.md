...
| `--yolo` | off | Skip the REPL's default safety gates: HITL approval is off instead of on, FileSystem/Shell/Git/Search are not sandboxed to the launch directory, and any `repl.safeMode` default from `fuseraft settings` is ignored. Restores the fully-open behavior for trusted, unattended sessions. Prints a warning banner when active. Make it the default with `fuseraft settings set repl.yolo true`. |
| `--include <dir>` | — | Add an additional allowed root directory alongside the launch directory, for a session spanning more than one project tree. Repeatable (`--include ../shared-lib --include ../other-service`). Shown in the startup banner as `Included:` and listed for the model in its system prompt. Ignored (with a warning) under `--yolo`, since there's no sandbox to add to. See [Filesystem sandbox](security.md#filesystem-sandbox) for how a path outside every allowed root is handled — a HITL grant on demand, not just a hard error. |
...
| `/goal resume [--max N]` | Pick up the last goal that did not complete, with a fresh audit budget (the same `N` it had, unless `--max` gives a new one). The goal is saved with the session, so this also works after `--resume` or `/switch`. A goal that is `paused` on a question doesn't need this: your next plain message answers it and continues the goal (the agent may ask again if the answer doesn't settle it). |
| `/goal drop` | Forget the last goal, so the next message is an ordinary turn instead of the answer to a paused goal. |
...
| `repl.yolo` | `true`/`false` — start every session as if `--yolo` were passed: HITL off, no filesystem/shell/git sandbox, `repl.safeMode` ignored (default `false`). Like the other REPL defaults it can't be turned off for one launch from the command line; run `fuseraft settings set repl.yolo false` instead |
