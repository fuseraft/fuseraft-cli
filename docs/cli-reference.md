...
| `/goal resume [--max N]` | Pick up the last goal that did not complete, with a fresh audit budget. A goal that is `paused` on a question doesn't need this: your next plain message answers it and continues the goal (the agent may ask again if the answer doesn't settle it). |
