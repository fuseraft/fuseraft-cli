...
**Cross-session linking.** Each session's chain is otherwise self-contained — a fresh `AuditLogger` per run, so `previous_hash` never spans files. To keep a project's audit trail from being N disconnected chains, a brand-new session (never a resumed one — that would re-inject a link into the middle of an existing chain) looks up this project's most recently written prior session and stamps its chain tip into its own first entry, as `agent_id: "system"`, `action: "SessionLinked:prev=<hash>"`. That claim is covered by the entry's own hash like any other field, so it's tamper-evident the same way; it isn't a real `previous_hash` link (the SDK doesn't expose a way to seed one), so confirming it means checking that some sibling session's file genuinely ends with the claimed hash — which is what `--verify` does. This proves the claimed predecessor state really existed in this project's history; it does not independently prove no session was skipped or that ordering wasn't manipulated by whoever controls the filesystem.

`fuseraft log audit` reads the chain back: with no flags it lists entries (filterable by `--session`, `--agent`, `--decision`, `--last`); with `--verify` it independently recomputes every hash, reports whether each file's chain is intact (exiting non-zero if any entry's stored hash, chain link, or sequence number doesn't match what's recomputed), and resolves any cross-session link claim against sibling sessions. `--verbose` output also includes governance events as they fire, for watching the chain build up live.
