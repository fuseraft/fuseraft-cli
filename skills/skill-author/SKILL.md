...
- Anything already covered by a shipped skill (`craft-orchestration`, `debug-session`, `config-audit`, `mcp-setup`, `skill-author`, `repl-tmux-driver`) or by a general-purpose skill in [fuseraft/skills](https://github.com/fuseraft/skills) (e.g. `commit`, `sandbox-test`, `build-docx`)
