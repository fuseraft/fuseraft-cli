...
using ContextAddCommand    = fuseraft.Cli.Commands.Context.ContextAddCommand;
using ContextListCommand   = fuseraft.Cli.Commands.Context.ContextListCommand;
using ContextRemoveCommand = fuseraft.Cli.Commands.Context.ContextRemoveCommand;
...
services.AddTransient<MemoryListCommand>();
...
app.Configure(cfg =>
{
    ...
    cfg.AddCommand<ReplCommand>("repl")
        ...
        .WithExample(["repl", "--model", "claude-sonnet-4-6", "--save"]);
    ...
    {
        branch.SetDescription("Persistent memory — REPL/agent facts (list/delete) and repository patterns extracted from evidence (review).");
        ...
        branch.AddCommand<MemoryListCommand>("list")
            .WithDescription("List stored REPL/agent memories.")
            .WithExample(["memory", "list"])
            .WithExample(["memory", "list", "--agent", "reviewer"]);
