...
/// <summary>
/// Experimental next-gen REPL.  Identical setup to <see cref="ReplCommand"/> but
/// delegates to <see cref="ReplNextTurn"/> for the terminal UI.
/// Enable as the default entry-point via <c>FUSERAFT_REPL_NEXT=1</c>, or invoke
/// directly with <c>fuseraft repl-next</c>.
/// </summary>
public sealed class ReplNextCommand(ILoggerFactory loggerFactory) : AsyncCommand<ReplSettings>
{
        ...
        CommandContext context, ReplSettings settings, CancellationToken cancellationToken)
    {
        ...
        // ── hand off to the next-gen turn loop ──────────────────────────────
        await ReplNextTurn.RunAsync(ctx, cancellationToken);
    ...
    // -------------------------------------------------------------------------
    // Private setup helpers (mirrored from ReplCommand)
    // -------------------------------------------------------------------------
    ...
    private ShellPolicy? TryLoadDefaultShellPolicy()
    {
        ...
        foreach (var path in candidates)
        {
            ...
            {
                loggerFactory.CreateLogger<ReplNextCommand>().LogDebug(
        ...
        bool jsonMode)
    {
        ...
        catch (Exception ex)
        {
            ...
            catch (Exception emitEx) { loggerFactory.CreateLogger<ReplNextCommand>().LogWarning(emitEx, "[SkillCuration] emitter failed: {Message}", emitEx.Message); }
        }
    }
}
